## Narrative Weaver - AI Narrative Engine

三层分离的叙事引擎 MVP，实现了：

- **Project / Context DB / Storyline** 三层模型
- **版本管理内核**：`Project.dbVersion` + `DbChangeLog` + Storyline 冲突标记
...
- `backend/`：叙事引擎后端（Node.js + TypeScript + Express + MongoDB）
  - `src/config/`：环境变量与 Mongo 连接
  - `src/models/`：Mongoose 模型
  - `src/services/`：业务服务（版本管理、Context Router 等）
...
- `tests/`：根级 Jest 测试
  - `helpers/testDb.ts`：基于 `mongodb-memory-server` 的测试 DB 管理
  - `unit/`：单元测试（versioning、模型 Schema、Context Router 等）
  - `integration/`：集成测试（CRUD + 版本流 + Agents Orchestrator）
- 根：
...
后端运行依赖 **MongoDB 实例**，例如本地：

```bash
docker run -d --name mongo-narrative -p 27017:27017 mongo
```

或使用你已有的 Mongo 实例，并在环境变量中配置连接串。
...
```bash
MONGODB_URI=mongodb://localhost:27017/narrative_weaver
PORT=4000
```

如使用其他地址/数据库名，可自行修改 `MONGODB_URI`。
...
- `tests/unit/versioningService.test.ts`  
  - `incrementProjectVersionAndLogChanges`  
  - `markAffectedStorylineNodes`（Character/Event/WorldRule 三种实体类型）
- `tests/unit/models-schemas.test.ts`  
  - `ProjectModel`：必填字段、`projectId` 唯一性  
  - `StorylineNodeModel`：`status` 枚举与默认值
...
- `tests/integration/context-db-api.test.ts`  
  ...
  - Characters / WorldRules / Events CRUD + `dbVersion` 递增断言  
  - StorylineNode CRUD + `status` 过滤
- `tests/integration/versioning-flow.test.ts`  
  - 完整版本工作流：  
    - 创建 Project / Character / Event / StorylineNode  
    - 修改 Character → `dbVersion` 增长、`DbChangeLog` 记录、StorylineNode 标记 `needs_revision`
...
> 注意：首次运行时，`mongodb-memory-server` 会下载 MongoDB 二进制（几十 MB），可能导致测试超时。  
> 如遇 `Exceeded timeout`，可在对应测试文件顶部调大 `jest.setTimeout(60000)` 或在 Jest 配置中设置更大的 `testTimeout`。
...
1. **启动 Mongo**（Docker 或本地实例）。  
2. **启动后端开发服务器**：`cd backend && npm run dev`。  
3. **手工调用 API**（Postman / curl）：
   - 创建 Project / Character / WorldRule / Event / StorylineNode
   - 调整 Context DB，观察 `dbVersion` 与 StorylineNode `needs_revision` 变化
   - 调用 `/bridger` 与 `/validator`，体验 Orchestrator 流程
4. **运行测试**：  
   - `npm run test:unit` 验证服务层逻辑  
   - `npm run test:integration` 验证端到端行为  
5. **迭代开发**：在此基础上接入真实 LLM（替换 `bridgerClient` / `validatorClient` 的 mock），或增加前端控制台。 
